...
AddEventHandler('fetchjob', function()
  ...
  AddEventHandler('drugs:sell', function()
	...
	local paymentc = math.random (1000,1200)
	local paymentw = math.random (800,1000)
	local paymentm = math.random (600,800)
	local paymento = math.random (1200,1400)
		...
		if coke >= 1 and success == true then
  		...
  		elseif weed >= 1 and success == true then
			...
			TriggerClientEvent("notifyc", source)
  			selling = false
			--callCops(copscalled) --Calls the cops
  		end
end)
...
function callCops (copscalled) --Calls the cops
	if copscalled == true then
		TriggerServerEvent('esx_phone: send', 'police', 'message', true, {x, y, z})
		--TriggerServerEvent(notifyc)
	else
		--Nothing
	end
end
